...
//
// Generate the bbcode cache PHP script
//
function generate_bbcode_cache()
{
	global $db;

	// Get bbcode from the DB
	$result = $db->query('SELECT * FROM '.$db->prefix.'bbcode') or error('Unable to fetch bbcode info', __FILE__, __LINE__, $db->error());
	...
	// Output bbcode list as PHP code
	$content = '<?php'."\n\n".'define(\'PUN_BBCODE_LOADED\', 1);'."\n\n".'$pun_bbcode = '.var_export($output, true).';'."\n\n".'?>';
	fluxbb_write_cache_file('cache_bbcode.php', $content);
}
